            ...
            <div className='p-4 shadow-xl bg-white border border-gray-100 h-[420px] lg:max-w-[450px] rounded-[10px] sm:p-6 lg:p-8 overflow-hidden'>
                ...
                <div className='flex items-center gap-3 my-3'>
                    <Button className="p-2 sm:p-6 lg:p-8" variant="outline" size="icon">
                        <Avatar>
                            <AvatarImage src={job?.company?.logo || job?.posted_by?.profile?.profilePhoto?.url || job?.posted_by?.profile?.profilePhoto} />
                        </Avatar>
                    </Button>
                    <div>
                        <h1 className='font-medium text-base sm:text-lg lg:text-xl'>{job?.company?.name || job?.posted_by?.fullname}</h1>
                        <p className='text-xs sm:text-sm lg:text-base text-gray-500'>
                            {job?.company?.location || job?.posted_by?.companyLocation || 'Location not specified'}
                        </p>
                    </div>
                </div>
